...
public class SimulateHandler {
    ...
    private static void gameLoop(BotState state, Game game, BlockingQueue<Optional<GameView>> queue, String id) {
    ...
    private static void waitLoop(BotState state, BlockingQueue<Optional<GameView>> queue, long delay, InteractionHook hook) {
        Runnable scheduled = () -> {
            try {
                ...
                if (optView.isPresent()) {
                    ...
                    waitLoop(state, queue, delay, hook);
                } else {
    ...
    public static void handle(BotState state, SlashCommandInteraction event) {
        ...
        if (finalDelay < MIN_DELAY || finalDelay > MAX_DELAY) {
            event.reply("Invalid delay, should be between " + MIN_DELAY + " and " + MAX_DELAY + " ms").queue();
            return;
        }
        ...
        EventUtils.replyView(event, startView, (hook) -> {
            ...
            gameLoop(state, startGame, queue, id);
            waitLoop(state, queue, finalDelay, hook);
        });
    }
}
